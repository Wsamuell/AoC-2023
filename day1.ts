...
const convertWordToNumber = (word: string): number => {
  ...
  for (let i = 0; i < word.length; i++) {
    ...
    if (Object.keys(numberMap).some((key) => currString.includes(key))) {
      let number = Object.keys(numberMap).find((key) =>
        currString.includes(key)
      );
      if (number !== undefined) {
...
let arrayOfWords: string[] = wordString.split('\n');
const grabNum = (arrayOfWords: string[]) => {
  let total = 0;
