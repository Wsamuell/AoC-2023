"use strict";
let wordString = `99lbqpxzzlbtvkmfrvrnmcxttseven
q7cnfslbtpkvseven
...
const convertWordToNumber = (word) => {
    ...
    for (let i = 0; i < word.length; i++) {
        const char = word[i];
        console.log(word, char, currString);
        if (/[a-z]/.test(char)) {
            currString += char;
        }
        else if (/\d/.test(char)) {
            arr.push(parseInt(char, 10));
            currString = '';
        }
        if (Object.keys(numberMap).some((key) => currString.includes(key))) {
            let number = Object.keys(numberMap).find((key) => currString.includes(key));
            if (number !== undefined) {
                arr.push(numberMap[number]);
                currString = '';
                i = i - 1;
            }
        }
    ...
    const joinNumbers = parseInt(firstNumber.toString() + lastNumber.toString(), 10);
    return joinNumbers;
};
// console.log(convertWordToNumber('xcntwone'));
let arrayOfWords = wordString.split('\n');
const grabNum = (arrayOfWords) => {
    let total = 0;
    for (let i = 0; i < arrayOfWords.length; i++) {
        convertWordToNumber(arrayOfWords[i]);
        total += convertWordToNumber(arrayOfWords[i]);
    }
    return total;
};
console.log(grabNum(arrayOfWords));
